...
- A `view` host plays on intersection, so a test asserting its clip is paused before a hover is racing the behavior the mode exists for. The jobtriage route declares `view`, and the standing lone Firefox failure comes from there. Measured across all three engines: Firefox answers `(hover: hover)` false, `(hover: none)` true, and both `(pointer: fine)` and `(pointer: coarse)` false, so it reports no pointer capability and every hover-keyed branch takes its touch path. `hover-video.ts` therefore plays on intersection under Firefox where the other two wait for a pointer. It reproduces with the pre-branch media file swapped in, and the codec, profile, pixel format, and level are identical across that change, so the media is not involved.
- Write a touch-only rule as `(pointer: coarse)` and a hover-only rule as `not all and (pointer: coarse)`. A coarse pointer is the one query that separates Firefox from a real phone, and requiring hover or a fine pointer strands Firefox on the touch branch without failing any assertion that exists.
