...
Owns how the three families reach the browser and how their sizes are named. Spans `src/layouts/base.astro` for loading and `src/styles/global.css` for the scale. Intended values live in `canon/DESIGN.md` § Typography, which this entry does not restate.
...
- The scale is named by role rather than by size. `--text-display`, `--text-heading`, `--text-lede`, `--text-body`, `--text-label`, and `--text-code` each carry their own line height, and weight where it differs from the family default.
- `--text-lede` fills the gap between heading and body. A case-study section opened with a paragraph at body size and separated itself from the paragraphs under it by color alone, which asks color to carry hierarchy that size should. The step takes the opening paragraph of each section and the deck under each route's title, one per section, so a reader meets the same size wherever a section starts.
