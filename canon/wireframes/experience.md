...
A lit chip wears the same treatment a pointed-at one wears, accent edge and glow, and the light was weaker than that until 2026-08-25. Timing is the whole difference now: the arrival swells and a pointer's answer snaps. A reader whose pointer rests on a chip during the wave's 1.48s therefore sees nothing change, which is the accepted cost of one accent across every bounded control on the page. `canon/ARCHITECTURE.md` § Two token sets carry elevation and response, and every control reads them, and `canon/context/motion.md` § The chip row's arrival, carry why the fill and the shadow could not carry that difference instead.

Clicking a chip glides to its card rather than jumping there, which the whole site now does from one declaration. See `canon/context/motion.md` § Traveling to a target.
