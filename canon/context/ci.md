...
The second consumer is `.github/evidence/surfaces/`, one desktop light still per landing section and case-study route (11 images), regenerated by `.github/workflows/pr-evidence-screenshot.yml` and its `bun run pr-evidence-screenshot` local counterpart. It takes the same shape as the README job: a fork guard, job-scoped `contents: write`, and a self-terminating push that retriggers the workflow and stops on the empty diff. It triggers broadly on `src/**` rather than one filter per surface, since eleven filters are eleven places a shared surface can be missed, at the cost of a doubled `verify.yml` run on most pull requests. The 56-image review-capture set stays gitignored and unchanged. A pull request touching both evidence sets gets one combined comparison comment, which nothing here has exercised yet.
