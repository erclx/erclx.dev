...
- `data-visible="true"` set at server render makes the fade a no-op. A project route used to set it on every faded element so those long-form pages rendered static, and that stopped being true at 858fcaf, which gave a route's prose the same reveal the landing page runs. Measured on `/aitk`, prose arrives between 778ms and 1111ms.
...
- Grouping suits a list that fits roughly within a screen, and only that. Measured at a 900px viewport: the timeline runs 674px and the closing ask 252px, so both group. The projects grid runs 1477px, which is 1.6 times the viewport, and stays on the batch path: watching its container would reveal the bottom cards while they are still off screen, and its ten faded elements would put the last one 1980ms out. A tall grid also delivers its cards in successive batches on its own, which is the case the batch stagger is good at.
...
- Four kinds of block stay static on purpose and every one of them is correct: the contact dock and the section rail run their own scroll gates, the shader canvas is a ground rather than an element, and the hero's fade band and the about surface's flight are decoration with animations of their own.
