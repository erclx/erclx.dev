...
- The portrait carries concentric contour rings behind it, swelling once every 13s and then resting. They are drawn in the field's own line weight so the photo reads as a feature in the terrain rather than an object on top of it, and they hang off a frame element because an image generates no pseudo-element of its own. See `.claude/context/motion.md` § Portrait rings.
