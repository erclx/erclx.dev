...
- It runs the hero's shader held to a single frame rather than an authored copy. There is one drawing, so the two cannot drift and there is no second rendering path to keep matched. See `canon/context/shader-field.md`.
...
- The weight is not a fraction of what a reader sees. See the alpha gotcha in `canon/context/shader-field.md`.
...
- Every buffer-side figure on this page was read before the composite repair, and that repair moved the quantity they measure without moving what a reader sees. The drawing buffer held `alpha squared` and now holds alpha raised to a per-theme curve, 1.79 in light and 3.45 in dark, so light's buffer readings rose and dark's fell while the composited surface stayed where it was. The guards all still pass, and the numbers above have not been re-read against the new buffer. Re-measure before treating any of them as current, and read composited pixels rather than buffer alpha for anything comparing across that repair. `canon/context/shader-field.md` carries the mechanism.
