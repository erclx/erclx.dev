...
- A power layer is a sibling drawn behind the member and it does not move when the member does, so a cell that stops at the silhouette tears a gap open the moment a behavior displaces the body. Such a cell runs 3 cells under the body, which covers the 13px the hop travels against the 54px smallest member.
...
- A rule swapping a tap power hid the resting layer on every member a reader touched, and one of the seven carries a replacement, so tapping any of the other six made its power vanish with nothing in its place. Scope a swap to the members that have something to swap to.
