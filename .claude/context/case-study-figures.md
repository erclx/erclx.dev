...
- The opened image fits the width and scrolls when it runs taller than the screen. Fitting to height instead caps a portrait chart at roughly a third of the width the screen offers, which returns a reader to the problem the treatment exists to solve. Measured 2026-08-15 on the baseline chart: 356px in the column, 488px fitted to height, 1168px fitted to width against a 1526px source.
