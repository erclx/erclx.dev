...
- The hover rule is written as `not all and (pointer: coarse)` rather than as a requirement for hover. See `canon/context/project-cards.md` for the engine that reports no pointer capability at all, which a hover-keyed rule strands on the touch path.
