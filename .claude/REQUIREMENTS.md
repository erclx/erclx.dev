...
- Authentication, comments, a dashboard or backend of this project's own
- Routes beyond the apex and one per shipped project
- Hand-written copy that diverges from canonical career sources

Analytics stopped being a non-goal on 2026-08-24. Cloudflare Web Analytics already runs on erclx.dev, enabled through the zone's own dashboard rather than through any code in this repository, and had been recording visits for months before the operator noticed. The line now bars a dashboard or backend this project builds and owns, which is the boundary worth keeping. A visitor count sourced from the host that already proxies the domain costs nothing here to build or maintain.
