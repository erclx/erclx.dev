...
Geist is removed. Replaced by Fraunces variable for display and headings, Inter variable for body and labels, JetBrains Mono variable for code. `canon/DESIGN.md` is the source of truth for token values. The tokens flow into Tailwind v4 via the `@theme` block in `global.css`.
...
`canon/context/agent-cast.md` holds, but about two things this run got wrong
...
The header drew a particle flow field on a 2D canvas. It is now a fragment shader rendering contours of a 3D noise stream function, lit by the gradient of its own line width. `canon/context/shader-field.md` carries the mount lifecycle and the uniform contract.
...
the band `canon/DESIGN.md` bars. The arms and the call site came out with the
...
nothing there. `canon/context/ci.md` carries that.
