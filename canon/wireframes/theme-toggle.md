...
- The control arrives with the hero rather than being in place before it. The marker sits on the button rather than on its slot in the header, because the handoff re-parents it into a fixed host on load and the slot it leaves collapses to a point. Verified at 1280x800: it fades in while rising 16px and still lands on the bar's slot at 0px in both axes. Mechanism: `canon/context/motion.md`.
