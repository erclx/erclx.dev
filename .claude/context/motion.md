...
- `e2e/lazy-images.ts` carries copies of the observer's `threshold` and `rootMargin`, since the walk a test runs has to wait on the elements this module is due to mark and neither value is exported. An edit to either here leaves those copies stale, and the walk then waits on an element nothing marks until its per-step timeout gives up.
