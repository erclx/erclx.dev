...
- The three values in that block are copies of what `:root` declares, and nothing ties the two together. Closing the duplication needs light-value aliases the dark block leaves alone, which is a token the design track owns rather than one this treatment may add, so the copy is the accepted cost and the block sits directly under the dark declarations to keep both in one view. A rewrite of the token layer has to carry the plate with it, since a rewrite that misses it leaves six figures on colors nothing else uses and no check reports the drift.
