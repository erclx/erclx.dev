...
- `canon gov sync` reverts `045-memory.md`'s fallback clause the same way. It names `.canon/memory/` as the write destination for a new memory file with no fallback to the old root, where `055-scratch.md` ships both the fallback and a `canon-keep-record-root` marker for the identical situation. That asymmetry is upstream, so the durable repair is a toolkit change and the local clause is a stopgap. Re-apply it after any sync, the same as `306-test-scope.md`, until the toolkit carries the fallback itself.
...
- The records move to `.canon/` is a two-step migration and the `.gitignore` change alone is not safe to merge ahead of the physical move. Removing the seven old ignore lines (`.claude/.tmp/`, `.claude/memory/`, `.claude/plans/`, `.claude/review/`, `.claude/groundwork/`, `.claude/intake/`, `.claude/tasks/`) while the folders they cover are still on disk turns every one of those files untracked the moment the change lands, which a review on the aitk-to-canon rename caught before merge: 963 files across the pen, the board, the review scratch, and the closed groundwork and intake trails. The fix is the same shape `055-scratch.md` already states for the scratch folder itself: keep both ignore spellings, and both path spellings in every hook that walks a record folder (`tasks-index.sh`, `memory-index.sh`, `standards-audit.sh`'s skip list), until `canon migrate records` has actually run. Only then do the old lines and the old path branches come out. A hook or an ignore entry keyed to one spelling alone is the same defect this file's standards-retirement entry above already collects: a guard reporting success the moment the path it watches stops existing.
