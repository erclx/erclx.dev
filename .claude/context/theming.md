...
- `--primary` keeps the operable role and gives up the state role. The rail's active marker and the timeline's current node moved to `--accent`, since both are computed from scroll position rather than raised by the reader. The rail label is a clickable anchor and still takes `--primary` for its focus ring, so one element carries a token from each role and the mark decides which, not the element.
