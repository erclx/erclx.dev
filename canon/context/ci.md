...
Do not close any of this by asserting something about the workflow file's contents. A check reading that `deploy` carries a setup step passes on a job that cannot run, which is the instrument class `canon/ARCHITECTURE.md` § What a check enforces here, and what nothing watches records and which shipped again this week. The instrument here is somebody firing the dispatch and reading the deployment URL out of the run log.
...
## A pull request comment depends on a path convention nobody had adopted
...
A preview runs behind a shorter gate than production, and the two are separate jobs for that reason. Production reaches the apex on a push to main behind `static-checks`, `unit-tests`, `build-verify`, and `e2e-tests`. A preview runs on a dispatch behind `build-verify` alone, since the artifact it publishes is already built by then and the browser matrix, which is the whole of the remaining wait, says nothing about whether a page renders. Measured on this branch across two dispatches. Behind the full gate the address printed 11 minutes after the dispatch, against the 15-minute ceiling `canon pr preview` waits out. Behind the build alone it printed in 83 seconds, with the browser matrix still running beside it.

That second number is when the preview is live, and it is not when the verb answers. `canon pr preview` waits on the whole run rather than on the deploy job, so it returns only once the matrix has finished either way, and the split moves the address from the eleventh minute to the second without moving the verb's reply. Read the alias out of the preview job's own log for a fast answer, and take the verb's reply as the slow, unattended one.

What makes the short gate safe is the ref fence rather than the tests. Both jobs deploy with `--branch=${{ github.ref_name }}`, so an unmerged ref lands on a preview host and only main reaches the apex.

The two jobs carry the same steps, and that duplication stands rather than moving into a composite action under `.github/actions/`. `canon pr preview` finds a project's deploy by scanning `.github/workflows/` for a workflow that runs `pages deploy` and carries a `workflow_dispatch` trigger, so a command extracted out of a workflow file leaves the verb reporting `no-deploy` against a repository that deploys perfectly well. This branch extracted it, measured that refusal, and put it back. Read the duplication as the detection contract's price.

The deploy job prints the preview address the same way, and did not until this branch. `canon pr preview` dispatches the deploy on a pull request's branch and then reads a `canon-preview-alias:` line out of the finished run's log, rather than computing an address from the branch name, since Cloudflare flattens a branch into its alias and may truncate it. This repository met three of that contract's four parts from the start, carrying a `workflow_dispatch` trigger and a deploy fenced with `--branch=${{ github.ref_name }}`, and printed no such line, so every preview call returned `no-alias`. The deploy step now carries an `id` and a step echoing its `pages-deployment-alias-url` output, gated to a dispatch, since a push to main lands on the apex and no reviewer needs told that address. The divergence was age rather than oversight: the cloudflare stack's own `deploy.yml` ships the step, and this job was hand-written before the verb that reads it existed.
