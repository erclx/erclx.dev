...
- The image draws the mark, the page header's line beside it, and a small `Eric Le · erclx.dev` attribution under that line. The line is `I build AI tools, mostly ones I needed myself first.`, held once in `scripts/card-copy.ts` so the card and the header cannot drift, and `src/test/rendered-copy.test.ts` asserts the header carries it. The name stays out of the image at display weight, since every host prints the title beside it. The description elaborates rather than repeats the line, since a host printing both beside each other would otherwise say the same words twice, and the same test fails when a page's description carries it.
