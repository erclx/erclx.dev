...
- Two creatures may not carry the same kind of feature. A bulb on the back and a shell on the back both read as a mass behind the head, so they were one silhouette drawn twice however different the masses were, and a one-sided stepped wing then read as the bushy tail for the same reason. The four that ship differ by where the weight sits: a quadruped with none, an up-swept tail, a pair of tall ears, and wings spread on both sides.
- Stance separates before a feature resolves. Three of the four creatures are long and low, so the winged one stands upright on two legs and is told apart at 44px before its wings are read at all.
