...
- The lock itself is no longer this module's. `lockPageScroll` and `releasePageScroll` moved to `src/lib/page-scroll-lock.ts` when the pronunciation route gained a second modal in its screenshot gallery, and `figure-zoom.ts` resolves them from there rather than declaring them. Two dialogs on one route holding equal copies of the scroll restore and the scrollbar-gap padding would drift, and nothing would report it.
...
- The three values in that block resolve through `--light-card`, `--light-muted-foreground`, and `--light-ring`, which `:root` declares and `.dark` leaves alone. The light declarations read the same aliases, so a light-palette edit reaches the plate on its own and the copies a rewrite could miss no longer exist. The block still sits directly under the dark declarations to keep both in one view. `canon/context/theming.md` § Token layers owns the alias group.
...
- The theme control is class-based, set on the root element by the first-paint script. A `prefers-color-scheme` query inside a `<picture>` would ignore a manual toggle, so a second chart file per theme is not available as the mechanism.
