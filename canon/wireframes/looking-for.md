...
On scroll-in the heading and the criteria rows reveal in a top-to-bottom cascade. The rows are watched as one list and step 220ms apart, so the cascade holds at any scroll speed rather than depending on which of them crossed the viewport edge together. Mechanism: `canon/context/motion.md`.
