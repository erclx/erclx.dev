...
- A pointer reading taken under `reducedMotion: 'reduce'` reports no response at any position, and reports it as byte-identical pixels rather than as an error. The still path draws one frame and attaches no pointer listener, so the preference that makes a capture reproducible is the one that makes a pointer question unanswerable. Pass `no-preference` whenever the question is what a pointer does, and read the preference back off the page rather than trusting the option that was passed.
- Reading the canvas back through `readPixels` on a second `getContext('webgl')` call returns zeros. The drawing buffer is cleared after compositing unless `preserveDrawingBuffer` was set on the original context, and a later request returns that same context rather than a fresh one. It reads as a surface drawing nothing.
...
- The noise's third axis reads `evolveAmplitude * sin(time * evolveAngularRate)` rather than unbounded time, because a value fed straight from `time` grows for as long as a tab stays open and eventually outgrows what a `mediump`-limited GPU can represent precisely. `highp` is optional in WebGL1 fragment shaders, so a GPU without it falls back to `mediump`, and the hash's `sin()`-based lattice lookup bands visibly once it loses too much precision, which reads as the contour pattern getting busier and harder-edged rather than as a brightness shift. Measured in `.canon/groundwork/01-shader-contour-consistency/08-spikes.md`, the drift crosses roughly 15% of a full cycle of representable error by 30 minutes of tab lifetime on a `mediump`-limited GPU and close to a full cycle by 3 hours, while a `highp`-capable GPU never drifts into this at any realistic tab lifetime. The bounded pair keeps the axis inside a small, fixed range for the life of the page instead, at the cost of the evolution easing to a stop and reversing near each extremum rather than holding a constant rate.
