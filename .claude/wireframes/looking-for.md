...
- A hairline rules each row rather than a gap separating them, and the section's own padding tightens around the block. The rows gave up their size step when the two columns settled, and the track a reader follows down the section is what replaces it. Measured 2026-08-19: the surface went from 584px carrying 188px of rows to 516px carrying 244px, so the share holding no row fell from 68% to 53%. The rules do more on a phone, where the stacked label and value ran together with nothing between them.
