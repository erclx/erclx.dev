...
- The hover-only form admits a `pointer: none` device, so a `view` host on a television or a kiosk binds a hover that never fires and shows its poster rather than its clip. Accepted knowingly: measured on all three engines, Playwright's Firefox answers `(pointer: none)` and `(hover: none)` both true and matches such a device on every query, so nothing separates them and a `(hover: none)` fallback would return Firefox to the intersection path this fix moved it off. Phones report a coarse pointer and keep the fallback they were given it for.
