...
How the landing page's scroll-triggered animation works. Layout and interaction intent for each surface live in its `canon/wireframes/<surface>.md` file. This entry covers the shared mechanism.
...
- The root declares `scroll-behavior: smooth` under `prefers-reduced-motion: no-preference`, and every control that scrolls resolves it rather than naming a behavior. That covers the rail, the bar's home control, and the timeline chips, which had answered the same question three different ways. `canon/ARCHITECTURE.md` § One declaration decides how the page travels, and every caller resolves it carries the reasoning and what it cost.
...
- The light is the chip's own hover, declared once for both states. A weaker arrival treatment was tried and reversed, since the border color carries nearly all of this site's glow response and two states sharing an edge color are otherwise indistinguishable.
...
- Under `prefers-reduced-motion: reduce` neither bar transitions and both still change shape, so that reader meets two shapes arriving instantly. See `canon/ARCHITECTURE.md` § One ground for two bars, and the shape moves while the row does not.
