...
`canon context audit` reads the cap clause above, and `bun run check:record` runs it with `--gate`. `scripts/verify.sh` calls it locally and the `static-checks` job calls it in CI, so a decision past the cap or a stale citation fails both. The verb is fetched with `bunx -y` and pinned to `@erclx/canon@4.114.0`, because 4.115.0 added a wireframe-sections rule that every wireframe fails. Moving the pin waits on the wireframes carrying the sections that standard requires. `bun run check` now needs the network, so a push from an offline machine with a cold `bunx` cache fails on a step unrelated to the change.
