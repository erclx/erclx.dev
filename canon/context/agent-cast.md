...
- `e2e/` owns the two instruments, `cast-motion.ts` and `cast-inventory.ts`, and the guards, split across `cast.spec.ts` and `cast-scheduler.spec.ts` with shared helpers in `cast-helpers.ts`. The split is a CI-timing concern, covered in `canon/context/ci.md`, and carries no meaning about the drawing.
...
- A mark is given to a member rather than carried by every mood. Binding a mark to a mood instead hands it to whatever figure takes that mood, which reads as one gesture repeated rather than as seven characters: seven of seven emitted on a tap and five of seven standing still. Three are drawn now, on three members, in one state each.
...
- The solo scheduler is a module at `src/components/site/experience/cast/scheduler.ts` rather than a loop inside `cast.astro`'s inline script, so its policies answer to an injected clock instead of a wall one. It takes the field element, a timer pair, a random source, and the three gates it cannot read for itself outside a browser, and it goes on querying the field for the pointer and the in-flight member rather than tracking either, which is the read-off-the-page design the source has always stated. That makes it an injected-DOM extraction rather than a pure one, and its tests run against a real element tree in the unit runner for the same reason.
- One of its three policies cannot leave the browser, and it is the tap gate. WebKit applies `:hover` to a tapped element and holds it, so ungated a single tap silences the cast for the life of the page, and jsdom matches `:hover` for nobody. The one-at-a-time rule and the reduced-motion gate are fake-clock tests in `scheduler.test.ts`. The tap case is the only test left in `e2e/cast-scheduler.spec.ts`, and it settles on the scheduler's next act rather than watching a window of them.
