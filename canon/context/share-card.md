...
- The image draws the mark with a small `Eric Le · erclx.dev` attribution under it and no sentence, since the career source carries none. The name stays out of the image at display weight, since every host prints the title beside it. The description elaborates rather than repeats the title, since a host printing both beside each other would otherwise say the same words twice.
